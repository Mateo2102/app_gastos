...
// Trae toda el área de datos (A1:N) de la hoja principal en una sola llamada. Se cachea
// unos segundos: una sola carga de página dispara varios endpoints (dashboard, histórico,
// gastos fijos, cuotas del mes...) que llaman a esto por separado, y sin cache repetían la
// misma lectura grande varias veces, agotando la cuota de lectura de Sheets.
let _gridCache = null;
let _gridCacheAt = 0;
...
async function getRawGrid() {
  const now = Date.now();
  if (_gridCache && (now - _gridCacheAt) < GRID_CACHE_MS) return _gridCache;
  const sheets = getSheetsApi();
  const title = await getDataSheetTitle();
  // Los datos en sí solo usan A:N, pero la tabla de referencia "Tarjeta/Cierre" puede estar en
  // cualquier columna más allá (se movió a P:Q al convertir el rango en una Tabla de Sheets),
  // así que leemos hasta Z para no depender de dónde quede ubicada.
  ...
  _gridCache = { title, values: resp.data.values || [] };
  _gridCacheAt = now;
  return _gridCache;
}
...
function parseDataRows(values, dolarBlue) {
  ...
  for (let i = start; i < values.length; i++) {
    ...
    const categoria = String(r[14] || '').trim(); // columna O: categoría de gasto (la define el usuario)
    ...
    rows.push({
      ...
      formaPago, banco, marca, fechaCierre, categoria,
...
export async function getOpciones() {
  ...
  for (let i = start; i < values.length; i++) {
    ...
    if (v) tipos.add(v);
    const c = String((values[i] || [])[14] || '').trim();
  ...
  const lista = rows
    ...
    .map(r => {
      ...
      return {
        ...
        formaPago: r.formaPago, banco: r.banco, marca: r.marca, categoria: r.categoria,
  ...
  await sheets.spreadsheets.values.update({
    ...
    requestBody: {
      values: [[
        ...
        formaPago, banco, marca, fechaCierreStr, String(data.categoria || '').trim()
      ]]
    }
  });
