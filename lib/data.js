...
// Trae toda el área de datos (A1:J) de la hoja principal en una sola llamada.
async function getRawGrid() {
  const sheets = getSheetsApi();
  const title = await getDataSheetTitle();
  const resp = await sheets.spreadsheets.values.get({
    spreadsheetId: CONFIG.SPREADSHEET_ID,
    range: `${title}!A1:J2000`,
...
// Busca la celda "Tarjeta" dentro de la grilla y arma el mapa Tarjeta -> n° de jueves de cierre
function getCierreMapFromGrid(values) {
  ...
  while (safety < 20 && r < values.length) {
    ...
    map[String(cardVal).trim()] = NTEXT[String(cierreVal).trim()] || 0;
    r++; safety++;
  }
  return map;
}
...
function parseDataRows(values, dolarBlue) {
  ...
  for (let i = start; i < values.length; i++) {
    ...
    const n = getCardKeyAndN(medio, cierreMap);
...
// Mapa Tarjeta -> n° de jueves de cierre (para armar la fila "hipotética" del simulador)
...
export async function getOpciones() {
  ...
  return {
    tarjetas: [...Object.keys(cierreMap), 'Efectivo'],
...
export async function agregarGasto(data) {
  ...
  await sheets.spreadsheets.values.update({
    spreadsheetId: CONFIG.SPREADSHEET_ID,
    range: `${title}!A${row}:J${row}`,
    valueInputOption: 'USER_ENTERED',
    requestBody: {
      values: [[
        data.gasto, data.tipo, data.medio, data.desc,
