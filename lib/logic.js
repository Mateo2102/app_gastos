...
// Cuotas de un pago que NO pasa por el ciclo de cierre de una tarjeta (transferencia, préstamo,
// efectivo): arrancan directamente en el mes de la compra, una por mes, sin corrimiento.
export function getInstallmentMonthsSimple(fechaCompra, cuotas) {
  return monthSequence(fechaCompra, cuotas);
}

// Meses de PAGO de una fila de "Gasto variable", eligiendo el algoritmo según cómo se pagó:
// tarjeta con fecha de cierre cargada a mano > tarjeta con cierre inferido (jueves N, legado) >
// transferencia/efectivo (mes de compra en adelante, sin ciclo de cierre).
export function getPaymentMonths(row) {
  if (row.formaPago === 'Tarjeta de Crédito') {
    const cierres = row.fechaCierre
      ? getClosingMonthsDesdeCierre(row.fechaCierre, row.cuotas)
      : getClosingMonths(row.fecha, row.n, row.cuotas);
    return cierres.map(k => shiftMonthKey(k, 1));
  }
  return getInstallmentMonthsSimple(row.fecha, row.cuotas);
}
